...
ヘッダーのタイトル横にある **「?」ヘルプアイコン**から、このアプリの使い方と、
**SQLite（実行エンジン）と Oracle で書き方が異なる点**（`ROWNUM` → `LIMIT` など）を
まとめた一覧を確認できます。

### SQL モード

- 基本 → WHERE → 並び替え → 集計 → グループ集計 → JOIN → サブクエリ、さらに更新系の
  `INSERT` / `UPDATE` / `DELETE`（`INSERT … SELECT` を含む）まで全 35 問。
...
- `INSERT` など更新系の問題は、**トランザクション内で実行 → 結果のテーブル状態を取得 →
  ROLLBACK** で元に戻して判定するため、サンプル DB の中身は変化しません。
...
| キー | 意味 |
| --- | --- |
| `cls` | 難易度の分類タグ（`lv-basic` `lv-sort` `lv-agg` `lv-group` `lv-join` `lv-sub` `lv-ins`。現状は表示に未使用のメタ情報） |
| `levelLabel` | サイドバーの問題一覧に表示するラベル |
| `q` | 問題文 |
| `hint` | 「ヒント」ボタンで表示する文章 |
...
DB を変更する問題は `type: "mutation"` とし、追加フィールドを指定します。トランザクション内で
実行し ROLLBACK で元に戻すため、サンプル DB は変化しません。答え合わせは「実行後のテーブル状態」
（`verify` の結果）同士の一致で判定します。

```json
{
  "type": "mutation",
  "levelLabel": "INSERT(基本)",
  "q": "products に新しい製品を1行追加してください…",
  "hint": "INSERT INTO products VALUES ( … )。",
  "answer": "INSERT INTO products VALUES (107, 'キーボード', '電子機器', 5000);",
  "verify": "SELECT * FROM products",
  "extraTokens": [
    { "title": "この問題の値", "cls": "val", "items": ["107", "'キーボード'", "'電子機器'", "5000"] }
  ]
}
```
...
| キー | 意味 |
| --- | --- |
| `type` | `"mutation"`（省略時は SELECT 問題） |
| `verify` | 実行後のテーブル状態を取得する SELECT。これの結果で照合します |
| `extraTokens` | この問題だけパレットに足す部品グループ（INSERT する値など）。`{title, cls, items}` の配列 |

`INSERT` `INTO` `VALUES` は基本パレットの「更新(INSERT)」グループに、テーブル名・列名は
「テーブル構成」にあります。
